...
def parameter_tuning_objective_func(X, *args):
    ...
    X_train, X_test, y_train, y_test = train_test_split(
        X, y, test_size=test_set_ratio, shuffle=False
    )
    ...
    params = {
        ...
        'min_child_weight': min_child_weight, #1, q
        'eta': eta, #0.1, 
...
def main():
    ...
    dir = './'
    filename = 'parsec__48_16_1_cpu'
    #filename = 'hashcat__8_8_1_gpu___1111'
    dataset = pd.read_csv(dir + filename + '.csv')
    keys= list(parameter.keys())  #in python 3, you'll need `list(i.keys())`
    values= list(parameter.values())
    #feature_names = list(df_X.columns)
    feature_names = [
      'Container_Memory_Usage(MB)',
      'Container_Memory_Mapped_File(MB)',
      'Container_Memory_Working_Set_Bytes(MB)',
      'Container_CPU_USAGE(%)',
      'Node_Load5',
      'Container_Memory_Limit(MB)',
      'Node_Procs_Running',
      'Node_Intr_Total(K)',
      'Container_CPU_Limit(%)',
      'Node_Procs_Blocked',
    ]
    dataset_X = dataset.loc[:, feature_names].copy()
    dataset_Y = dataset['Active_Power(W)'].copy()
    df_X = dataset_X.copy()
    df_Y = dataset_Y.copy()
    test_set_ratio = 0.5
...
# l
# nohup /media/linux/anaconda3/envs/data_process/bin/python DE_xgboost_cpu_power_prediction.py > XGBoost-output-cpu-power-$(date "+%Y-%m-%d-%H-%M-%S")
