...
def main():
    ...
    params = {
        ...
        "bagging_freq": 23,
        "max_depth": 5,
        "feature_fraction": 0.7671144368294283,
        "bagging_fraction": 0.9837038231026893,
        "learning_rate": 0.05053443763355425,
        "early_stopping_rounds": 86,
        "n_important_features": 20,
        "num_leaves": 61,
        "max_bin": 131,
        "min_data_in_leaf": 96,
        "min_split_gain": 0.43979066770046055,
        "lambda_l1": 0.17984177051789596,
        "lambda_l2": 0.2159092699155915,
    }
    ...
    dtrain = lgb.Dataset(
        data=X_train, label=y_train, feature_name=list(dataset_X.columns)
    )
    dtest = lgb.Dataset(data=X_test, label=y_test, feature_name=list(dataset_X.columns))
    ...
    feature_names = list(
        np.array(bst.feature_name())[sorted_idx][:n_important_features]
    )
    ...
    dtrain = lgb.Dataset(data=X_train, label=y_train, feature_name=feature_names)
    dtest = lgb.Dataset(data=X_test, label=y_test, feature_name=feature_names)
    ...
    data_dmatrix = lgb.Dataset(X_train, label=y_train, feature_name=feature_names)
    ...
    score = lgb_cv_result["train rmse-mean"][-1]
    print("score", score)
