...
def main(args):
    ...
    params = {
        "n_estimators": 345,
        "min_child_weight": 2,
        "eta": 0.16197443276063908,
        "colsample_bytree": 0.5535566929550477,
        "max_depth": 5,
        "subsample": 0.906726378003857,
        "lambda": 0.46744228090822737,
        "learning_rate": 0.0959403426086924,
        "early_stopping_rounds": 57,
        "n_important_features": 24,
    }  
    ...
    print(
        "The root mean squared error (RMSE) on test set: {:.4f}".format(
            np.sqrt(mean_squared_error(dtest.get_label(), bst.predict(dtest)))
        )
    )
    print(
    ...
    print(
        "The R square (R2) on test set: {:.4f}".format(
            np.sqrt(r2_score(dtest.get_label(), bst.predict(dtest)))
        )
    )
    print(
    ...
    shap_importance.to_csv("CPU_shap_importance.csv", index=False)
